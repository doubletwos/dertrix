...
namespace Dertrix.Controllers
{
    public class StudentSubjectsController : Controller
    {
        ...
        public ActionResult Grades(int? id, int? ij, string searchname, string searchid)
        {
            ...
            var j = db.Classes.Where(s => s.OrgId == id && ij == s.ClassRefNumb).Select(g => g.ClassRefNumb).FirstOrDefault();
            ...
            {
                return View(db.StudentSubject.Where(p => p.ClassRef == j && p.OrgId == i).Include(s => s.Subject).Include(r => r.RegisteredUser).ToList());
            }
